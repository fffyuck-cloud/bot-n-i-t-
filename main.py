...
def build_game_embeds(game, title, last_player_name=None):
    ...
    # Dùng chr(96) để tránh tuyệt đối lỗi cú pháp khi copy code qua lại
    TICKS = chr(96) * 3
    history_box = f"{TICKS}fix\n{history_str}\n{TICKS}"
...
async def start_game_vi(ctx):
    ...
    await ctx.send(embed=build_game_embeds(games[ctx.channel.id], "NỐI TỪ TIẾNG VIỆT (MULTIPLAYER)"))
...
async def start_game_vi_bot(ctx):
    ...
    await ctx.send(embed=build_game_embeds(games[ctx.channel.id], "NỐI TỪ TIẾNG VIỆT (VS BOT)"))
...
async def start_game_en(ctx):
    ...
    await ctx.send(embed=build_game_embeds(games[ctx.channel.id], "ENGLISH WORD CHAIN (MULTIPLAYER)"))
...
async def start_game_en_bot(ctx):
    ...
    await ctx.send(embed=build_game_embeds(games[ctx.channel.id], "ENGLISH WORD CHAIN (VS BOT)"))
    ...
    if game["mode"] == "vi":
        ...
        if game["vs_bot"]:
            ...
            if bot_word:
                ...
                game["count"] += 1
                await message.channel.send(embed=build_game_embeds(game, "NỐI TỪ TIẾNG VIỆT (VS BOT)", last_player_name="🤖 Bot Trí Tuệ"))
            else:
            ...
            await message.channel.send(embed=build_game_embeds(game, "NỐI TỪ TIẾNG VIỆT", last_player_name=message.author.display_name))
        ...
        if game["vs_bot"]:
            ...
            if bot_word:
                ...
                game["last_word"] = bot_word
                game["count"] += 1
                await message.channel.send(embed=build_game_embeds(game, "ENGLISH WORD CHAIN (VS BOT)", last_player_name="🤖 English Bot"))
            else:
                win_embed = discord.Embed(
            ...
            await message.channel.send(embed=build_game_embeds(game, "ENGLISH WORD CHAIN", last_player_name=message.author.display_name))
